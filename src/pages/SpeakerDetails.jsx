...
export const speakerDetailsLoader = async ({ params }) => {
  ...
  const speaker = speakers.find((p) => p.id === parseInt(params.id));
  if (!speaker) {
      ...
      <main className="h-full w-screen flex flex-col py-20">
        ...
        <div className="speaker-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2  my-auto lg:mb-16  mx-8 lg:gap-4 items-center  justify-center  lg:mx-28 rounded-md ">
          ...
          <div className=" h-full w-full ">
            <picture>
              <source
                srcSet={speaker.image.desktop}
                media="(min-width: 1024px)"
              />
              <source
                srcSet={speaker.image.tablet}
        ...
        <div className="pictures grid grid-cols-1 md:grid-cols-10 lg:grid-cols-10 mx-8 my-auto lg:mb-16 gap-6 lg:gap-4  lg:mx-28 rounded-md">
          ...
          <div className="h-full w-full lg:h-full lg:w-full  lg:col-span-4 flex flex-col gap-10  ">
            <picture className="picture-1 ">
              <source
                srcSet={speaker.gallery.first.desktop}
                media="(min-width: 1024px)"
                className="h-[200px]"
              />
              <source
                srcSet={speaker.gallery.first.tablet}
                media="(min-width: 768px)"
                className="h-[200px]"
              />
              <img
            ...
            <picture className="picture-2 ">
              <source
                srcSet={speaker.gallery.second.desktop}
                media="(min-width: 1024px)"
                className="h-[200px]"
              />
              <source
                srcSet={speaker.gallery.second.tablet}
                media="(min-width: 768px)"
                className="h-[200px]"
              />
              <img
                src={speaker.gallery.second.mobile}
                alt=""
                className="h-[200px] rounded-md"
              />
            </picture>
          ...
          <div className="  rounded-sm text-gray-950 lg:col-span-6 ">
            <picture className="Big-picture">
              <source
                srcSet={speaker.gallery.third.desktop}
                media="(min-width: 1024px)"
              />
              <source
                srcSet={speaker.gallery.third.tablet}
                media="(min-width: 768px)"
              />
              <img
                src={speaker.gallery.third.mobile}
                alt=""
